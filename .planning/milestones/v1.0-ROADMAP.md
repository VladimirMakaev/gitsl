...
GitSL transforms git commands into Sapling equivalents, enabling tools like get-shit-done to work transparently with Sapling repos. The roadmap builds from core execution infrastructure (parsing, subprocess handling) through command translation layers (direct mappings, then flag transformations) to output format emulation for tooling compatibility. E2E testing is foundational - we built the test harness early and used golden-master comparison (git vs gitsl output) to validate every phase.
